...
class PiTesterGUI:
    ...
    def get_automatic_tests(self):
        """
        Automatic tests are tests that do not require manual observation.
        Example: UART, SPI, Wi-Fi.
        """
        automatic_tests = []
    ...
    def confirm_setup_checklist(self, test):
        ...
        This is where switch-position confirmation will be used later.
        For now, it asks the user to confirm that the hardware switch bank
        is set correctly for the selected test.
        """
        ...
        def show_popup():
            ...
            instructions = test.get("instructions", "No instructions provided.")

            switch_checklist = test.get("switch_checklist", [])

            if switch_checklist:
                checklist_text = "\n".join(
                    [f"• {item}" for item in switch_checklist]
                )
            else:
                checklist_text = (
                    "• Placeholder: Confirm the hardware switch bank is set correctly.\n"
                    "• Placeholder: Confirm GPIO ports are routed to the proper test peripheral.\n"
                    "• Placeholder: Confirm the selected test peripheral is connected.\n"
                    "• Specific switch positions will be added later."
                )

            message = (
                f"Before running: {test_name}\n\n"
                "Setup / Switch Checklist:\n"
                f"{checklist_text}\n\n"
                "Test instructions:\n"
                f"{instructions}\n\n"
                "Click Yes to continue with this test.\n"
        ...
        return response_holder.get("confirmed", False)
